...
public class Pause_stageselect : MonoBehaviour {
	...
	public void Up ()
	{
		...
		else
		{
			...
			if(m_fTime >= m_fWaitTime)
			{
				...
				// ステセレボタンの更新を有効にする
				GameObject.Find("Pause_back").GetComponent<Pause_back>().UpdateActivate();
			}
		}
	...
	// ステージセレクトボタンを描画
	public void DrawPause_stageselect()
	{
		m_fSita = 0.0f;							// 角度を初期化
		GetComponent<RectTransform>().localScale =
			new Vector3 (m_fMinScale, m_fMinScale, m_fMinScale);	// 拡大率を初期化

		m_bUpdate = true;						// 最初ステセレボタンの更新を無効
	...
	// ステージセレクトボタンを描画しない
	public void UnDrawPause_stageselect()
	{
